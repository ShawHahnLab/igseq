...
class TestGetreads(TestBase):
    ...
    def make_mock_bcl2fastq(cls):
        ...
        def side_effect(args, extra_args=None):
            ...
            (path/"Stats").mkdir()
            getreads.LOGGER.info("bcl2fastq command: %s", args)
    ...
    def test_getreads(self):
        ...
        with TemporaryDirectory() as tmpdir:
            with self.assertLogs(logger=getreads.LOGGER, level=logging.INFO) as logcm:
                # TODO need to actually make _run_bcl2fastq pull in the
                # subprocess' stderr/stdout, otherwise this won't work
...
class TestGetreadsMissingFiles(TestGetreads):
    ...
    def test_getreads(self):
        ...
        # files
        self.assertEqual(logcounts, {"INFO": 6, "CRITICAL": 2})
...
class TestGetreadsExtraFiles(TestGetreads):
    ...
    def test_getreads(self):
        ...
        # files
        self.assertEqual(logcounts, {"INFO": 6, "WARNING": 1})


class TestGetreadsLive(TestGetreads, TestLive):
    """Basic test of getreads with actual bcl2fastq.

    Everything here uses the same exact setup as TestGetreads above, except
    since it's a live test the mock isn't used, and the real input files for
    this class *are* used.
    """
