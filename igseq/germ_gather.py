"""
Gather germline reference sequences into one directory.
"""
...
def gather_germline(db_paths, dir_path_out, dry_run=False):
    """Gather V/D/J FASTA into one directory as V.fasta, D.fasta, J.fasta."""
    # front-end for _gather_germline with support for internal paths
    ...
    if paths["type"] == "dir":
        if not dry_run:
            _gather_germline(paths["path"], dir_path_out)
    elif paths["type"] == "internal":
        if not dry_run:
            _gather_germline(paths["path"], dir_path_out)

def _gather_germline(dir_path_in, dir_path_out):
